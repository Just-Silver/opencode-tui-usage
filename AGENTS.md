...
- 额度按 `providerID` 分桶：`model/quota.ts` 的 `QuotaStore`（cache/at/inFlight，依赖注入可测），`quota/index.ts` 的 `PROVIDER_API_URL`/`QUOTA_INTEGRATIONS`/`fetchers` 注册表，加供应商只需新增 `quota/<provider>.ts` 并注册；白名单守卫与 fetcher 分发均经 `isQuotaProvider`/`normID` 按「小写+只留 [a-z]」归一化匹配（用户写法 `opencode-go`/`opencodego`/`opencode_go` 均命中），配置/DB 查找保持原样 pid（同源天然命中）；API URL 经 `getProviderApiUrl` 归一化查找（未注册落 `QUOTA_API_URL` 兜底）
- Command Code 支持已实现（`quota/command-code.ts`：`mapCommandCode` 纯函数 + `fetchCommandCode` 双 API），**注册暂注释**（`quota/index.ts` 内 `COMMAND_CODE_INTEGRATION`/`QUOTA_INTEGRATIONS`/`PROVIDER_API_URL`/`fetchers` 四处 + import，取消注释即启用）；测试 `tests/command-code.test.ts` 离线覆盖转换逻辑
