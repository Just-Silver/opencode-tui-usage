...
test("自动发现：enabled:false 的 command-code 被跳过", () => {
  assert.equal(isQuotaProvider("command-code"), false)
  assert.equal(isQuotaProvider("commandcode"), false)
})

test("自动发现：URL 查找与兜底正常", () => {
  assert.equal(getProviderApiUrl("opencode-go"), "https://opencode.ai/zen/go/v1/usage")
  assert.equal(getProviderApiUrl("command-code"), QUOTA_API_URL) // 未启用 → 兜底
  assert.equal(getProviderApiUrl("nope"), QUOTA_API_URL)
})